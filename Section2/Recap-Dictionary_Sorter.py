...
The function should sort the dictionary by its values in ascending order and return
a new dictionary containing the sorted key-value pairs.
...
def dictionary_sorter(data_dict):
    sorted_dict = {}
    for key in sorted(data_dict, key=data_dict.get):
        sorted_dict[key] = data_dict[key]
    return sorted_dict


print(dictionary_sorter({'b': 1, 'c': 2, 'a': 3}))
